...
class GunParams(BaseModel):
    napiecie_kv: float = Field(..., ge=0, le=100, description="Zakres zgodnie z CM-10 s. 34")
    prad_ua: float = Field(..., ge=0, le=500, description="Instrukcje CM-10 s. 35 ograniczają prąd")
    przeplyw_kg_min: float = Field(..., ge=0, le=10, description="CM-21 s. 42 definiuje maksymalny przepływ")
